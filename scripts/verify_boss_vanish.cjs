#!/usr/bin/env node
/* verify_boss_vanish.cjs -- the Act 1 boss must be a LIVE sprite over real floor and leave NOTHING
 * behind when defeated (owner, build 67: "the shadow does not remain even after defeating it").
 * For each of the four boss floors: seed a save standing two cells south of the boss, (a) boss
 * alive -> the live sprite exists (a1dBossSprites has a sprite, no mask); (b) boss.<id>.defeated set
 * -> no boss sprite, and the canvas at the boss anchor reads as FLOOR: its luminance is within 40 of
 * the plate's own pixel there. On the build-70 dist (b) fails: the dark cover ellipse reads ~20.
 *   node scripts/verify_boss_vanish.cjs [http://127.0.0.1:5179/] */
...
(async () => {
  ...
  for (const [mapId, b] of Object.entries(BOSS)) {
    // stand two cells south of the boss so it is on screen and she is not on it
    await boot(save(mapId, b.stand[0], b.stand[1], {}));
    const alive = await probe(mapId, b, 'alive');
    await boot(save(mapId, b.stand[0], b.stand[1], { ['boss.' + b.id + '.defeated']: true }));
    const dead = await probe(mapId, b, 'defeated');
    check(`${mapId}: standing on floor 3 with the boss cell on screen`, alive.floor === 3 && dead.floor === 3 && alive.sx > 0 && alive.sx < 390 && alive.sy > 0 && alive.sy < 844, JSON.stringify({ alive: alive.floor, dead: dead.floor, tile: alive.tileAtBoss, at: [alive.sx, alive.sy] }));
    check(`${mapId}: the live boss is drawn while alive (dark mark at the anchor, luminance < 60)`, typeof alive.lum === 'number' && alive.lum < 60, `lum alive=${alive.lum} rgb=${alive.rgb}`);
    const floorLum = await platePixel(mapId, b);
    check(`${mapId}: after defeat the boss anchor reads as the plate's own FLOOR (within 25 of the plate pixel)`, typeof dead.lum === 'number' && Math.abs(dead.lum - floorLum) <= 25, `lum alive=${alive.lum} defeated=${dead.lum} plate=${floorLum} rgb=${dead.rgb} at (${dead.sx},${dead.sy})`);
  }
  await browser.close();
