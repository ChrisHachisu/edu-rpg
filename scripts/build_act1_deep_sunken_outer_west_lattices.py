#!/usr/bin/env python3
"""Build deterministic shared-lattice overlays for the Relay 07 outer batch."""
...
BATCH = RUNTIME / "deep-sunken-outer-west-912-v1"
WORLD_SIZE = 512
...
REGIONS = [
    {
        ...
        "x": 224,
        "y": 2240,
        "master": BATCH / "sunken-deep-authored-master-v1.png",
        "composition": BATCH / "composition-refs/sunken-deep-composition-512.png",
    },
    {
        "id": "millbrook-outer-west-912-v1",
        "slug": "millbrook-outer-west",
        "x": 1152,
        "y": 1952,
        "master": BATCH / "millbrook-outer-west-authored-master-v1.png",
        "composition": BATCH / "composition-refs/millbrook-outer-west-composition-512.png",
    },
]
...
METRICS = BATCH / "deep-sunken-outer-west-lattice-metrics-v1.json"
CONTACT_SHEET = BATCH / "deep-sunken-outer-west-lattice-contact-sheet-v1.png"
SHARED_COMPOSITE = BATCH / "deep-sunken-outer-west-shared-composite-912-v1.png"
...
def build_shared_composite() -> tuple[Image.Image, dict[str, Image.Image]]:
    ...
    for region in REGIONS:
        ...
        box = (left, top, left + LATTICE_SIZE, top + LATTICE_SIZE)
        if coverage.crop(box).getbbox() is not None:
            raise AssertionError(f"Relay 07 regions unexpectedly overlap: {region['id']}")
        canvas.paste(reduced[region["slug"]], (left, top))
        coverage.paste(Image.new("L", (LATTICE_SIZE, LATTICE_SIZE), 255), (left, top))
...
def main() -> None:
    assert (UNION_LEFT, UNION_TOP, UNION_RIGHT, UNION_BOTTOM) == (224, 1952, 1664, 2752)
    assert UNION_SIZE == (2565, 1425)
    ...
    metrics: dict[str, object] = {
        "pipeline": {
            ...
            "overlapPairs": [],
            "overlapPolicy": "directed crossfade only for authored-region overlaps; this batch has none",
            "sharedComposite": "two disjoint authored regions on one global 57/32 lattice",
            "paletteReduction": f"deep-sunken-outer-west batch global Pillow MEDIANCUT {PALETTE_COLORS} colors, no dithering",
    ...
    print("ACT 1 DEEP-SUNKEN/OUTER-WEST LATTICES BUILT: 2 authored regions on one deterministic 57/32 lattice")
