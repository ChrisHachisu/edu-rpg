...
# STATE AS OF 2026-08-21 — this supersedes the 08-20 section above

Everything the 08-20 section was waiting on is DONE. Both plates are baked, stitched, installed and
pinned; payload is **722**; all gates are green. One gate band still fails and `TOWN_IDS` is still
not flipped, which is the only thing between here and playable.

## The anchor question is CLOSED

It held on all seven remaining tiles. Every tile cleared the 0.60 layout floor (+0.65 to +0.88) and
both plates cleared the mean and hard-step floors. The "denser primer" fallback in the 08-20 section
was never needed and should not be attempted.

## What the PAINTERLY failure actually was

Both plates failed one band, soft steps over a 40% ceiling — while measuring HARDER than the
accepted plate. That is not the painting failure mode (v6 had hard 13.9%). Rendered side by side the
cause was obvious: **the towns were EMPTY**. Props make hard steps, bare ground makes soft stipple,
so soft% was reporting density, not finish. Cause was upstream of every brief: `PLAN` had ten
entries and none was a prop.

Fixed by `scatter_clutter()` in `scripts/town_layout.py` — props ONLY on ground the walkable body
already refuses, because collision here is authored first and a barrel on paving is a barrel the
player walks through. 108 props on millbrook, 120 on greenhollow, zero pixels touching paving.

|  | mean | hard | soft | flat | paved-not-walkable |
|---|---|---|---|---|---|
| portSapphire, accepted | 22.17 | 29.7% | 38.8% | 26.9% | 56.7% FAIL |
| millbrook | 22.95 | 33.9% | 43.0% | 17.0% | 39.5% |
| greenhollow | 23.83 | 35.4% | 42.1% | 16.3% | 46.0% |

Both towns now beat the accepted plate on mean AND hard, and both PASS the LAYOUT gate it fails.

## Do not re-derive these

- **`run()` in `town_layout.py` rewrites the walkable authority** as a side effect of writing the
  primer. It regenerated BYTE-IDENTICAL here (md5 checked before and after), which is why the pins
  held and the walk verifications did not need re-running. **Check that md5 every time.**
- **A tile is not the shipping unit.** greenhollow tile 0,1 measured 16.85 mean, under the floor;
  the per-tile exposure match lifted its plate to 23.83.
- **The residual soft is NOT portSapphire's harbour.** Measured over land only the accepted plate is
  39.4%, still inside the ceiling. The remainder is the open yard and the wide paving, which are
  walkable and so are the one place the dead-zones-only rule cannot put anything.
- **`codex exec` sub-agents write stray redraws into the WORKING DIRECTORY**, not only
  `~/.codex/generated_images`. They are not pipeline output; delete them.
- **A pin placeholder must be 64 LITERAL zeros**, never `"0" * 64` — `regenerate_pins.py` matches
  `[0-9a-f]{64}` inside the quotes, so an expression is invisible to it and the next step fails with
  a misleading "Act 1 town identity mismatch".

## What is left

1. **OWNER DECISION: the PAINTERLY band.** Both plates are one band short of clean. Options put to
   the owner and still open: accept and flip, or spend more generations on the yard.
2. **Flip `TOWN_IDS` in `public/act1-hifi/adapter.js`** — one line, and the moment it lands a player
   can walk into both towns. Do it LAST and only on the owner's word.
3. **STEP 4, the entry walk, has still never been run.** `scripts/greenhollow_verify_entry.cjs` and
   `scripts/millbrook_verify_ingame.cjs` both need `TOWN_IDS` set first. Everything else is proven.
