...
### Same defect class, found by sweeping, NOT fixed with the terrain work

Both change **gameplay rules** rather than terrain, so each deserves its own change and its own
verification rather than riding along with a loader fix.

| # | Defect | Status |
|---|---|---|
| L1 | `a1aLandmarkTex` sets `a1aLmLoading[key]=2` on one transient image error, commented *"permanent: owSpecialObjects runs at 12.5 Hz"*. The cap is there to avoid a retry storm, but the effect is that a landmark can vanish for the whole session — and **dungeon/town entrances ARE landmarks**, so a transient failure can hide an entrance | **OPEN** — wants the same bounded back-off as the layer loader (which solves the 12.5 Hz concern the flat cap was reaching for) |
| L2 | `a1mMaskFor` sets `a1mMasks[key]=null` on one error, commented *"no mask -> engine steps"* — i.e. that floor's pixel-accurate collision is **silently replaced by coarse engine tile rules** for the session | **OPEN, and it bears on S5.** A floor running on tile rules would let the hero stand where the pixel mask forbids it, which is a **candidate mechanism for "the player can walk on top of the arch" appearing INTERMITTENTLY**. Rule this out before commissioning the mouth re-bake — an art fix cannot repair a mask that was never loaded |
