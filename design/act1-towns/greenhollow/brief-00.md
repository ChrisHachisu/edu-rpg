...
WHAT IS IN THIS TILE, AND WHAT EACH COLOUR MEANS HERE:
  ...
  the block with the dull red upper band in the middle right is a BUILDING. The coloured part is its ROOF and the brown strip below it is its FACADE, so it faces DOWN-SCREEN. Give it a door on that facade and windows, and stand it exactly on its block. It is CUT OFF by the edge of this tile: draw only the part that is here, right up to the edge, and do not complete it.
  the block with the slate blue upper band in the lower centre is a BUILDING. The coloured part is its ROOF and the brown strip below it is its FACADE, so it faces DOWN-SCREEN. Give it a door on that facade and windows, and stand it exactly on its block.
  the grey disc in the lower right is a STONE WELL. Only the part of it inside this tile is drawn here.
