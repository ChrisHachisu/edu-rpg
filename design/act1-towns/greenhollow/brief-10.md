...
WHAT IS IN THIS TILE, AND WHAT EACH COLOUR MEANS HERE:
  ...
  the block with the slate blue upper band in the upper centre is a BUILDING. The coloured part is its ROOF and the brown strip below it is its FACADE, so it faces DOWN-SCREEN. Give it a door on that facade and windows, and stand it exactly on its block.
  the grey disc in the upper right is a STONE WELL. Only the part of it inside this tile is drawn here.
  the tan gap in the palisade in the lower right is THE ONE GATE of the village, and the only way in or out. Draw a real gateway there: posts, a lintel, open leaves.
